...
- **BREAKING**: `AddPayloadBodyPlugin` / `AddRadarPlugin` 打包请求体前过滤 payload（剔除 `_` 前缀控制参数与 `null` 值，对齐 PHP artful `filter_params`）：`_unpack_raw` 等控制参数不再进入发往网关的请求体（银联等全字段验签网关下多出的字段会导致验签失败）；响应解包侧仍传全量 payload（对齐 PHP `guide()` 第三参语义）
- 与 PHP artful 的有意差异：`XmlPacker` 遇非法 XML 返回结构化错误（`XmlDeserializeError`）而非 PHP 侧的 `InvalidArgumentException`；XML pack 遇嵌套容器显式报错而非产出 PHP 的 `<![CDATA[Array]]>` 垃圾值；XML pack 空集合产出 `<xml></xml>`（与 PHP 一致）；XML 叶子文本一律字符串（PHP simplexml → json 全程无数字转换）
